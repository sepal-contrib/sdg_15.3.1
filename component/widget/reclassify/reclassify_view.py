    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.title = v.CardTitle(
            children=[v.Html(tag="h2", children=[cm.rec.rec.title])]
        )

        # create the input widgets
        self.w_input_title = v.Html(
            tag="h2", children=[cm.rec.rec.input.title], class_="mt-5"
        )

        if self.gee:
            self.w_image = sw.AssetSelect(label=cm.rec.rec.input.asset, folder=folder)
        else:
            self.w_image = sw.FileInput(
                [".tif", ".vrt", ".tiff", ".geojson", ".shp"],
                label=cm.rec.rec.input.file,
            )

        self.w_code = v.Select(
            label=cm.rec.rec.input.band.label,
            hint=cm.rec.rec.input.band.hint,
            v_model=None,
            items=[],
            persistent_hint=True,
        )

        w_optional_title = v.Html(tag="h3", children=[cm.rec.rec.input.optional])
        ...
        self.w_class_title = v.Html(
            tag="h2", children=[cm.rec.rec.input.classif.title], class_="mt-5"
        )
        ...
        self.w_table_title = v.Html(
            tag="h2", children=[cm.rec.rec.table], class_="mt-5"
        )
        ...
        self.get_table = sw.Btn(
            cm.rec.rec.input.btn, "mdi-table", color="success", small=True
        )
        ...
        self.reclassify_btn = sw.Btn(
            cm.rec.rec.btn, "mdi-checkerboard", small=True, disabled=True
        )
