...
def test_the_corpus_has_28_uniquely_named_scenarios():
    assert len(SCENARIOS) == 28


def test_the_custom_land_cover_arm_carries_a_genuinely_custom_scheme():
    ...
    scheme = SCENARIOS["s05"].land_cover.scheme
    assert scheme is not None
    assert scheme.is_custom is True
    assert SCENARIOS["s09"].land_cover.scheme is None


def test_every_scenario_sets_a_vi_threshold():
...
def test_every_level_of_every_axis_appears_at_least_once():
...
def test_every_scenario_names_a_level_on_every_axis():
...
def test_sensor_by_index_is_covered_exhaustively():
...
def test_land_cover_by_water_mask_is_covered_exhaustively():
...
def test_every_scenario_round_trips_through_json(name):
    spec = SCENARIOS[name]
    assert RunSpec.from_dict(spec.to_dict()) == spec


def test_scenario_fingerprints_are_all_distinct():
