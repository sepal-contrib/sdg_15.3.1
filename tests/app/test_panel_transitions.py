...
def test_the_panel_renders_before_a_run(monkeypatch):
    ...
    box, rc = solara.render(
        TransitionsPanel(maps=maps, ctx=ctx, gee_interface=None), handle_error=False
    )
...
def test_clicking_compute_fetches_and_mounts_the_real_chart_option(monkeypatch):
    ...
    async def main():
        ...
        box, rc = solara.render(
            TransitionsPanel(maps=maps, ctx=ctx, gee_interface=None),
            handle_error=False,
        )
        assert rc is not None
...
def test_the_task_uses_the_snapshot_taken_at_click_time_not_a_live_read(monkeypatch):
    """A Build that lands WHILE Compute transitions is still running must not
