...
class ClassTable(v.DataTable, sw.SepalWidget):
    ...
    def __init__(self, out_path=Path.home() / "downloads", **kwargs):
        ...
        self.delete_btn = sw.Btn(
            cm.rec.table.btn.delete, icon="mdi-delete", color="error", small=True
        )
        self.add_btn = sw.Btn(
            cm.rec.table.btn.add, icon="mdi-plus", color="success", small=True
        )
        self.save_btn = sw.Btn(
            cm.rec.table.btn.save, icon="mdi-content-save", small=True
        )
    ...
    def _remove_event(self, widget, event, data):
        ...
        current_items = self.itecm.copy()
        current_itecm.remove(self.v_model[0])
...
class EditDialog(v.Dialog):
    ...
    def __init__(self, table, **kwargs):
        ...
        # Action buttons
        btn_txt = cm.rec.table.edit_dialog.btn
    ...
    def _modify(self, widget, event, data):
        ...
        current_items = self.table.itecm.copy()
    ...
    def _save(self, widget, event, data):
        ...
        current_itecm.insert(0, item_to_add)
...
class TableView(v.Card, sw.SepalWidget):
    ...
    def __init__(
        ...
        self.btn = sw.Btn(
            cm.rec.table.classif.btn, icon="mdi-table", color="success", outlined=True
        )
